...
class AWQQuantizerXL:
    def __init__(
        ...
        self.timing_stats: Dict[str, float] = {
            "awq_origin_seconds": 0.0,
            "smart_flip_seconds": 0.0,
        }
    ...
    def quantize_layer(self, name: str, module: nn.Linear):
        ...
        if self.post_correction is not None:
            ...
            self._sync_cuda()
            origin_dequant_t0 = time.perf_counter()
            w_quant = quant_state.dequantize_truncated()
            self._sync_cuda()
            origin_t1 = time.perf_counter()
            self.timing_stats["awq_origin_seconds"] += origin_t1 - origin_dequant_t0

        w_final = (w_quant / best_scales.unsqueeze(0)).to(original_dtype)
        module.weight.data = w_final
        if self.post_correction is not None:
            self.timing_stats["awq_origin_seconds"] += origin_t1 - origin_t0
    ...
    def quantize_lmhead_half_by_half(self, name: str, module: nn.Linear, debug: bool = False, num_chunks: int = 4):
        ...
        if self.post_correction is None:
            ...
            self._sync_cuda()
            flip_mean_t0 = time.perf_counter()
            _, raw_mean = self.get_activation_stats(name)
            if raw_mean is None:
                post_mean = torch.zeros(in_features, device=self.device, dtype=w.dtype)
            else:
            ...
            self._sync_cuda()
            flip_mean_t1 = time.perf_counter()
            self.timing_stats["smart_flip_seconds"] += flip_mean_t1 - flip_mean_t0
    ...
    def quantize_model_sequential(self, calibration_data, n_samples: int = 500):
        ...
        print(f"    AWQ origin (search + quantize): {self.timing_stats['awq_origin_seconds']:.2f}s")
        print(f"    Smart flip (post-correction): {self.timing_stats['smart_flip_seconds']:.2f}s")
        print(
            f"    Total quantization: {self.timing_stats['awq_origin_seconds'] + self.timing_stats['smart_flip_seconds']:.2f}s"
        )
        print("=" * 80)
