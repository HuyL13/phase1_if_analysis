...
def prepare(config_path: str, python: str) -> None:
    ...
    if config['quantizer'] != 'awq':
        return
    repo = Path(config['upstream_ptq_repo']).resolve()
    wrapper = repo/'tools'/'run_awq_upstream.py'
    calibration = Path(config['calibration']).resolve()
    if not wrapper.is_file():
        raise FileNotFoundError(f'Pinned upstream wrapper not found: {wrapper}')
    ...
    for seed in config['seeds']:
        for variant in ('clean', 'fingerprinted'):
            ...
            command = [python, str(wrapper), '--model-path', str(source),
